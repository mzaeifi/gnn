...
def assignVar():
    conf = {
        'src_fl_nm': DATA_DIR+'fastentity/source_fast.json',
        'trgt_fl_nm': DATA_DIR+'fastentity/target_fast.json',
        'op_fl_nm': DATA_DIR+'output/word_sim/word_sim'
    }
    return conf
###############
def getDataParam():
...
################
#################### Main Code START ####################
def genWordSim(data_param):
    try:
        print("#################### GenWordSim START ####################")
        strt_tm = datetime.datetime.now()
        conf = assignVar()
        word_sim_ind = data_param["sim_ind"]
